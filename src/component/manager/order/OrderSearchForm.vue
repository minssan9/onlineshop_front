<template>
  <div class="input-group mt-5 ml-auto mr-auto" style="max-width: 500px">
    <div class="input-group-prepend">
      <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown"
              aria-haspopup="true" aria-expanded="false">{{menu}}
      </button>
      <div class="dropdown-menu">
        <a v-for="(item,index) in menus" :key="index" class="dropdown-item" @click="menuChange(item)" href="#">{{item}}</a>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit() {
        ...
        this.FETCH_ORDER_MANAGER_SEARCH({page: 0, keyword: this.keyword, option: option})
      }
    }
  }
</script>
